...
router.get('/:id', authenticate, (req, res) => {
  const db  = getDb();
  const row = db.prepare(`${FM_SELECT} WHERE f.id = ?`).get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Feuille introuvable.' });
  res.json(row);
});

// POST /api/feuilles — Enregistrer une feuille (médecin)
router.post('/', authenticate, requireRole('medecin'), (req, res) => {
  const db = getDb();
  ...
  // Récupère le médecin lié à cet utilisateur
  const med = db.prepare('SELECT id FROM medecins WHERE utilisateur_id=?').get(req.user.id);
  if (!med) return res.status(403).json({ error: 'Compte non lié à un médecin.' });

  // Vérif doublon (même assuré + même date + même médecin)
...
router.patch('/:id/statut', authenticate, (req, res) => {
  ...
  const feuille = db.prepare('SELECT * FROM feuilles_maladie WHERE id=?').get(req.params.id);
...
router.patch('/:id/completer', authenticate, requireRole('assureur'), (req, res) => {
  const db = getDb();
  ...
  db.prepare(`UPDATE feuilles_maladie SET montant_remboursement=?, mode_paiement=?, notes=COALESCE(?,notes),
